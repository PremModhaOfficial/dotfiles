...
update_jcode() {
  ...
  if [ -n "$current" ] && [ "$current" != "$GATE_BRANCH" ]; then
    run "checkout back to $current" git -C "$JCODE_SRC" checkout "$current"
  ...
  # Build the dev binary (bounded; user can also run `jcode self-dev --build`).
  run "cargo build jcode" cargo build --manifest-path "$JCODE_SRC/Cargo.toml" --bin jcode
...
update_tools() {
  ...
  if command -v node >/dev/null; then
    run "babysitter SDK probe" node -e "require('child_process').execSync('npm exec --yes --package @a5c-ai/babysitter-sdk@latest -- babysitter --version', {stdio:'inherit'})"
  fi
...
sync_configs() {
  ...
  if [ -d "$DOTFILES/jcode/.jcode/hooks" ]; then
    run "install jcode hooks" cp -R "$DOTFILES/jcode/.jcode/hooks/." "$HOME/.jcode/hooks/"
  ...
  if [ -f "$DOTFILES/herdr/plugins/config/pickr/config.toml" ]; then
    ...
    run "install pickr config" cp "$DOTFILES/herdr/plugins/config/pickr/config.toml" "$HOME/.config/herdr/plugins/config/pickr/config.toml"
  fi
  ...
  # Commit + push the dotfiles repo (our changes ride along with config updates).
