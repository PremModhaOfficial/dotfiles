...
return {
	...
	config = function()
		...
		if vim.fn.executable("go") == 0 then
			vim.notify("Go executable not found in PATH. Testing will not work.", vim.log.levels.WARN)
		end
		...
		if pcall(require, "dap") then
			...
			if not dap.configurations.go then
				vim.notify("DAP not configured for Go. Debug tests with <leader>td may not work.", vim.log.levels.WARN)
			end
		end
	end,
	keys = function()
